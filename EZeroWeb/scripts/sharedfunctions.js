...
function setupList(data) {
   ...
   //$("#dataStore").html(data);
    ...
    $(data).find('station').each(function (index, val) {
        ...
        if (currCity != ctyName) {
            if (alpha) {
                ...
                if (currAlpha != newAlpha) {
                    ...
                    var alphaDiv = "<div class='listAlpha sixteen columns alphaDiv' ><div class='reverseBox alphaLetter'>" + currAlpha + "</div></div>"
                    listHTML += alphaDiv;
                }
            }
            listHTML += "<div class='sixteen columns mainColor currCity'><div style='margin-left:5px'>" + currCity + "</div></div>";
        }
        else {
        ...
        var upperDiv = "<div class='stationItem'>hi there<div class='one-third column upperDiv'><div class='stName'>" + unescape(stName) + "</div></div>"
        listHTML += upperDiv;
        ...
        var middleDiv = "<div class='one-third column middleDiv'><div class='addr'>" + unescape(addr) + "</div><div class='mapLink'><div style='cursor:pointer;' onclick='initMap(" + mapContent + ")'><img class='mapIcon' src='images/maplocation_30.png'/></div></div></div>"
        listHTML += middleDiv;

        var lowerDiv = "<div class='one-third column lowerDiv'><div style='float:left;margin:4px 0px 0px 0px;'><span class='brandTag'>Brand: </span>" + sBrand + "</div><div class='octane'>Octane: " + octane + "</div></div></div>"
        listHTML += lowerDiv;
